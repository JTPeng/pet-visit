# 宠物上门喂养小程序 - 实现计划总览 (Master)

> 本文件是**所有实施计划的总追踪索引**。每完成一个子计划中的任务，请勾选本文件中对应的任务项。
>
> **对应需求文档**：[../specs/2026-05-09-pet-visit-mvp-design.md](../specs/2026-05-09-pet-visit-mvp-design.md)
>
> **执行策略**：B + Master —— 写完 1 份 → 实施 → 写下一份；Master 提供全局蓝图。

---

## 项目结构

```
pet-visit/
├── serve/           # NestJS 后端（所有端共用）
├── wx-app/          # uni-app 小程序前端（宠主端）
├── admin/           # Vue 3 + Vant 移动端 H5 管理后台
└── docs/
    └── superpowers/
        ├── specs/   # 需求设计文档
        └── plans/   # 本目录：实施计划
```

---

## 阶段总览

| 阶段 | 计划文件 | 状态 | 依赖 |
|---|---|---|---|
| P1 基础骨架 | [01-foundation](./2026-05-09-01-foundation.md) | 🟡 已编写，待实施 | 无 |
| P2 用户体系 | `2026-05-09-02-user-auth.md`（待编写） | ⚪ 未开始 | P1 |
| P3 文件上传 | `2026-05-09-03-upload-oss.md`（待编写） | ⚪ 未开始 | P1, P2 |
| P4 内容审核 | `2026-05-09-04-moderation.md`（待编写） | ⚪ 未开始 | P1, P2 |
| P5 宠物档案 | `2026-05-09-05-pet-profile.md`（待编写） | ⚪ 未开始 | P1, P2, P3, P4 |
| P6 社区功能 | `2026-05-09-06-community.md`（待编写） | ⚪ 未开始 | P1, P2, P3, P4 |
| P7 管理后台 | `2026-05-09-07-admin.md`（待编写） | ⚪ 未开始 | P1-P6 |

**图例**：⚪ 未开始 · 🟡 进行中 · ✅ 已完成

---

## P1：基础骨架

**目标**：搭建前后端工程骨架，所有后续模块可立即接入。

- [ ] P1-T1 后端：核心依赖安装与目录结构
- [ ] P1-T2 后端：环境配置模块（ConfigModule + dotenv）
- [ ] P1-T3 后端：Prisma 接入 + 数据库连接
- [ ] P1-T4 后端：全局响应拦截器（统一 API 响应格式）
- [ ] P1-T5 后端：全局异常过滤器（统一错误格式）
- [ ] P1-T6 后端：日志模块（pino）
- [ ] P1-T7 后端：参数校验管道（class-validator 全局启用）
- [ ] P1-T8 后端：CORS 中间件
- [ ] P1-T9 后端：健康检查接口
- [ ] P1-T10 后端：docker-compose（PostgreSQL 开发环境）
- [ ] P1-T11 前端：依赖安装（Pinia、持久化插件）
- [ ] P1-T12 前端：目录结构整理（api/stores/utils/types/components）
- [ ] P1-T13 前端：环境变量配置（dev/prod）
- [ ] P1-T14 前端：请求封装（uni.request + Token 注入 + 错误处理）
- [ ] P1-T15 前端：Pinia 接入 + 用户 store 骨架
- [ ] P1-T16 前端：3 Tab 导航配置（pages.json）
- [ ] P1-T17 前端：3 个 Tab 页面骨架（首页 / 发帖 / 我的）
- [ ] P1-T18 前端：全局通用样式（主题色、通用变量）
- [ ] P1-T19 联调：前端调用后端健康检查接口
- [ ] P1-T20 README：更新 serve/README.md 与 wx-app 启动文档

**完成标准**：后端启动 → 健康检查接口返回 200；前端启动 → 3 Tab 可切换 → 首页调用健康接口成功。

---

## P2：用户体系

**目标**：微信登录 + JWT + 手机号绑定 + 用户资料 + 关注关系。

- [ ] P2-T1 后端：User 数据模型（Prisma schema）
- [ ] P2-T2 后端：微信登录接口（code → openid → 用户创建/登录）
- [ ] P2-T3 后端：JWT Token 签发与验证中间件
- [ ] P2-T4 后端：全局 AuthGuard 与 @Public 装饰器
- [ ] P2-T5 后端：手机号绑定接口（微信 encryptedData 解密）
- [ ] P2-T6 后端：用户资料接口（获取/更新昵称、头像、简介）
- [ ] P2-T7 后端：关注关系模型与接口（关注/取消/列表）
- [ ] P2-T8 后端：限流装饰器（发帖/评论等场景用）
- [ ] P2-T9 前端：登录流程（静默登录 → 存 Token）
- [ ] P2-T10 前端：登录态 store + 自动刷新机制
- [ ] P2-T11 前端：手机号绑定弹窗组件（触发时机：发帖/评论前）
- [ ] P2-T12 前端：个人资料编辑页
- [ ] P2-T13 前端：他人主页
- [ ] P2-T14 前端：关注/取消关注交互
- [ ] P2-T15 联调 + 端到端测试

---

## P3：文件上传（OSS/COS）

**目标**：图片直传对象存储，前端通过后端获取临时凭证。

- [ ] P3-T1 后端：OSS/COS 选型确认（腾讯云 COS 或阿里云 OSS）与账号准备
- [ ] P3-T2 后端：临时凭证接口（STS / 直传签名）
- [ ] P3-T3 后端：上传回调校验（可选）
- [ ] P3-T4 前端：图片选择 + 压缩工具
- [ ] P3-T5 前端：直传工具函数 + 进度反馈
- [ ] P3-T6 前端：头像上传组件（用户/宠物复用）
- [ ] P3-T7 前端：多图上传组件（社区发帖复用）
- [ ] P3-T8 联调 + 路径规则校验

---

## P4：内容审核

**目标**：微信 msgSecCheck/imgSecCheck + 本地敏感词库 + 举报 + 封禁。

- [ ] P4-T1 后端：微信 AccessToken 管理（缓存 + 刷新）
- [ ] P4-T2 后端：msgSecCheck 封装
- [ ] P4-T3 后端：imgSecCheck 封装
- [ ] P4-T4 后端：本地敏感词库 + 过滤工具
- [ ] P4-T5 后端：双审服务（本地词 → 微信接口）
- [ ] P4-T6 后端：审核队列模型（通过/疑似/违规）
- [ ] P4-T7 后端：举报模型与接口
- [ ] P4-T8 后端：违规处罚（警告 → 禁言 → 封号）
- [ ] P4-T9 后端：用户封禁状态校验（全局 Guard）
- [ ] P4-T10 前端：举报弹窗组件
- [ ] P4-T11 前端：被封禁提示页
- [ ] P4-T12 联调 + 审核流程测试

---

## P5：宠物档案

**目标**：极简宠物档案（头像/昵称/品种/性别/生日/体重 + 备注），多宠管理。

- [ ] P5-T1 后端：Pet 数据模型（含 extra_info JSONB、公开开关预留）
- [ ] P5-T2 后端：宠物 CRUD 接口
- [ ] P5-T3 后端：品种字典接口（猫/狗/其他）
- [ ] P5-T4 前端：我的宠物列表页
- [ ] P5-T5 前端：宠物新增/编辑表单
- [ ] P5-T6 前端：宠物详情页
- [ ] P5-T7 前端：在"我的"页展示宠物卡片
- [ ] P5-T8 联调

---

## P6：社区功能

**目标**：图文发帖、信息流（推荐/关注）、评论、点赞、收藏、话题标签。

- [ ] P6-T1 后端：Post/Comment/Interaction 数据模型
- [ ] P6-T2 后端：话题标签模型（硬编码 + 接口）
- [ ] P6-T3 后端：发帖接口（审核前置）
- [ ] P6-T4 后端：推荐流接口（热度 × 时间衰减）
- [ ] P6-T5 后端：关注流接口（时间倒序）
- [ ] P6-T6 后端：帖子详情接口
- [ ] P6-T7 后端：评论接口（发布/列表/删除）
- [ ] P6-T8 后端：点赞接口（帖子/评论）
- [ ] P6-T9 后端：收藏接口
- [ ] P6-T10 后端：分页与限流
- [ ] P6-T11 前端：首页信息流（推荐/关注切换）
- [ ] P6-T12 前端：帖子卡片组件
- [ ] P6-T13 前端：发帖页（图文 + 话题 + 关联宠物）
- [ ] P6-T14 前端：帖子详情页
- [ ] P6-T15 前端：评论组件（楼中楼）
- [ ] P6-T16 前端：点赞/收藏交互
- [ ] P6-T17 前端：分享卡片
- [ ] P6-T18 前端：收藏列表页
- [ ] P6-T19 联调 + 端到端

---

## P7：管理后台（移动端 H5）

**目标**：独立 `admin/` 工程，基于 Vue 3 + Vite + TypeScript + Pinia + Vant，方便管理员在手机上处理审核、举报、封禁等敏感操作。

- [ ] P7-T1 后端：Admin 数据模型（独立于 User，账号/密码哈希/角色）
- [ ] P7-T2 后端：管理员登录接口（账号密码 + JWT，签发独立的 admin Token）
- [ ] P7-T3 后端：AdminAuthGuard（与普通 AuthGuard 分离）
- [ ] P7-T4 后端：审核队列接口（`/api/admin/moderation/*`）
- [ ] P7-T5 后端：举报处理接口（`/api/admin/reports/*`）
- [ ] P7-T6 后端：敏感词维护接口（`/api/admin/sensitive-words/*`）
- [ ] P7-T7 后端：用户封禁接口（`/api/admin/users/*/ban`）
- [ ] P7-T8 后端：管理员种子脚本（创建初始管理员）
- [ ] P7-T9 前端：`admin/` 工程初始化（Vite + Vue 3 + TS）
- [ ] P7-T10 前端：Vant 组件库接入 + 移动端 viewport 配置
- [ ] P7-T11 前端：Pinia + 路由 + 请求封装（与 wx-app 相同思路，独立 Token key）
- [ ] P7-T12 前端：登录页
- [ ] P7-T13 前端：主框架（Tabbar：审核 / 举报 / 词库 / 我的）
- [ ] P7-T14 前端：审核队列页（列表 + 详情 + 通过/拒绝）
- [ ] P7-T15 前端：举报列表页（列表 + 处理）
- [ ] P7-T16 前端：敏感词维护页（新增/删除/搜索）
- [ ] P7-T17 前端：用户封禁页（搜索 + 封禁/解封）
- [ ] P7-T18 部署：独立子域名 + Nginx 配置（可选 IP 白名单 / Basic Auth）
- [ ] P7-T19 联调 + 端到端测试

---

## 追踪规则

1. **完成一个任务项，立即在本文件和对应子计划里勾选 `[x]`**
2. **每完成一个子计划的所有任务，修改本文件阶段表格状态为 ✅**
3. **发现新问题或新需求**：
   - 若影响当前阶段 → 在对应子计划里追加任务
   - 若影响未来阶段 → 在本文件对应阶段下备注
4. **提交规范**：每完成一个任务独立 commit；每完成一个阶段打 tag `p{N}-done`
