...
**目标**：图片直传对象存储，前端通过后端获取临时凭证。

- [ ] P3-T1 后端：OSS/COS 选型确认（腾讯云 COS 或阿里云 OSS）与账号准备
- [ ] P3-T2 后端：临时凭证接口（STS / 直传签名）
- [ ] P3-T3 后端：上传回调校验（可选）
- [ ] P3-T4 前端：图片选择 + 压缩工具
- [ ] P3-T5 前端：直传工具函数 + 进度反馈
- [ ] P3-T6 前端：头像上传组件（用户/宠物复用）
- [ ] P3-T7 前端：多图上传组件（社区发帖复用）
- [ ] P3-T8 联调 + 路径规则校验
